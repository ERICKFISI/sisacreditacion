...
public function mostrar_Facultad_idUsusario() 
{
    $obj=new Main();
    $datos=$obj->mostrar_Facultad_idUsusario($idusuario);
    return $datos;
    
}
public function leer_sub_eventos($idevento) {
...
//        $obj->table = $p['table'] no uso tabla;
     ...
     public function grilla($name, $columns, $rows, $options, $pag, $edit, $view, $select = false, $new = true,$asig,$chek,$presidente,$cheket_estado_asignacion=false) {
        $obj = new Main();
        $data = array();
        $data['nr'] = $obj->getnr();
        $data['cols'] = $columns;
